...
Before we begin, we use the set.seed() function in order to set a seed for seed R’s random number generator, so that we will obtain precisely the same results as those shown below. It is generally a good idea
...
Next, we use the boot() function to compute the standard errors of 100 bootstrap estimates for the intercept and slope terms.
